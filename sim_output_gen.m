function ret = sim_output_gen(Excel, flightdata, forces, propinfo, avionics_data, max, Prop, Eng, Roc)
...
Excel.Range('AC1:AL1').Select();
Excel.Selection.Value = {'Time (s)', 'Acceleration (m/s^2)', 'Velocity (m/s)', 'Altitude (m)', 'Projected Apogee (m)', 'Distance', 'Elevation Angle', 'Rocket to GSE Angle', 'FSPL', 'Link Margin'};
[rows, ~] = size(avionics_data);
Excel.Range(sprintf('AC2:AL%i', rows)).Select();
Excel.Selection.Value = num2cell(avionics_data);
